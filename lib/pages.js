...
// Interim ordering while online card checkout is being connected.
// Public business number — shown on the site as the WhatsApp order line.
const WHATSAPP_NUMBER = "923038032042";
function waOrderLink(message) {
...
<a class="btn btn-wa buy-full" style="margin-top:10px" target="_blank" rel="noopener"
href="${waOrderLink(`Hi LittleLearners! I'd like to order the "${p.name}" pack (£${(p.price_minor / 100).toFixed(2)}).`)}">Order on WhatsApp</a>
<p class="muted" style="margin-top:10px;font-size:14px">Online card checkout is being set up — order on WhatsApp now and we'll send your download link after payment.</p>
<p id="buyNote" class="muted" style="margin-top:10px"></p>
<ul class="buy-trust">
<li>✓ Download link sent after payment</li>
<li>✓ Safe, simple ordering</li>
...
document.getElementById('buyBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    note.textContent = 'Online checkout is being connected — you can order right now on WhatsApp above!';
...
(function(){
  ...
  function render(){
    ...
    '<a class="btn btn-wa" id="waOrderBtn" target="_blank" rel="noopener" style="margin-top:14px">Order on WhatsApp</a>' +
    '<p class="muted" style="margin-top:8px">Online card checkout is being set up — send your order on WhatsApp and we\'ll arrange payment and send your download links.</p>';
    var waLines = cart.map(function(s){ var p = bySlug[s]; return '• ' + p.name + ' — ' + money(p.price); });
    document.getElementById('waOrderBtn').href =
      'https://wa.me/${WHATSAPP_NUMBER}?text=' + encodeURIComponent('Hi LittleLearners! I\'d like to order:\n' + waLines.join('\n') + '\nTotal: ' + money(total));
    ...
    list.querySelectorAll('[data-buy]').forEach(function(b){
      b.addEventListener('click', async function(){
        ...
        try {
          ...
          var j = await r.json();
          if (j.checkoutUrl) { location.href = j.checkoutUrl; return; }
          alert('Online checkout is being connected — use the WhatsApp order button below instead.');
